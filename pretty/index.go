...
func (p PrettyUtils) Pretty(json []byte) []byte {
	return p.PrettyOptions(json, nil)
}

func (p PrettyUtils) GetDefaultOptions() *Options {
	return defaultOptions
}

// PrettyOptions is like Pretty but with customized options.
func (p PrettyUtils) PrettyOptions(json []byte, opts *Options) []byte {
	if opts == nil {
...
func (PrettyUtils) Ugly(json []byte) []byte {
	buf := make([]byte, 0, len(json))
...
// slice upon return.
func (PrettyUtils) UglyInPlace(json []byte) []byte { return ugly(json, json) }
...
// TerminalStyle.
func (PrettyUtils) Color(src []byte, style *Style) []byte {
	if style == nil {
		style = TerminalStyle
	}
	apnd := style.Append
